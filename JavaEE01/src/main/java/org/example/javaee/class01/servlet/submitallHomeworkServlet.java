...
@WebServlet("/submitallHomework")
public class submitallHomeworkServlet extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        try {
            if (StudentHomeworkJdbc.addStudentHomework(newsHomework)) {
                ...
                //显示弹窗并且当关闭弹窗后跳到指定页面
                resp.getWriter().write("<script>alert('提交失败，请仔细检查后再提交！网页将跳转到提交界面！'); window.location='submitallHomework.jsp'; window.close();</script>");
                resp.getWriter().flush();
            }
